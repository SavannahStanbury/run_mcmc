...
t0=REACH.date
times=np.arange(t0,t0+1.,1./24)
for i in range(len(times)):
    print(ephem.date(times[i]))
'''
...
mode = 'HE'
filename='leda-dipole-emp.npz'

a_H = []
a_E = []
for f in freqs:
    H = a_value(filename, f , 'HE')
    H = np.array(H)
    a_H.append(H[0])
    a_E.append(H[1])

np.savez('TABLE_coef_a.npz', freq=freqs, a_E=a_E, a_H=a_H)
...
def func_LWA(nu,theta,phi,aE,aH):
    ...
    pE = np.abs(p_i(nu, theta, aE))  # Ensure no negative values inside log     
    pH = np.abs(p_i(nu, theta, aH))

    log_term1 = 2 * np.log(pE) + np.log(np.cos(phi)**2)
    log_term2 = 2 * np.log(pH) + np.log(np.sin(phi)**2)

    # Use log-sum-exp trick to prevent overflow
    max_log = np.maximum(log_term1, log_term2)
    result = np.exp(0.5 * (max_log + np.log(np.exp(log_term1 - max_log) + np.exp(log_term2 - max_log))))
...
#Quick check beam
filename='TABLE_coef_a.npz'
nside=64

def generate_beam(freq, time):
  beam=gen_beam_LWA(nside,time,freq,a_fit(filename,freq,'E'),a_fit(filename,freq,'H')) #for LST 6
  for pix in range(len(beam)):
    if beam[pix]==0: beam[pix]=hp.UNSEEN
  return beam

#beam = generate_beam(70,time)

#hp.mollview(beam,coord=['G','C'], min = 0, max = 1)
#hp.graticule()
#plt.show()
